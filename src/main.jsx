...
function LoginPage({ onBack, onOpenSignup, onSubmit }) {
  ...
  return (
    <main className="authPageMain authPageMain--locked">
      <DeferredParticles />
...
function App() {
  ...
  const [menuExpandedByFilter, setMenuExpandedByFilter] = useState({});
  ...
  const menuDisplayLimit = 5;
  const menuExpansionKey = `${selectedRestaurant}::${activeCategory}`;
  const isMenuExpanded = Boolean(menuExpandedByFilter[menuExpansionKey]);
  const visibleMenuItems = isMenuExpanded
    ? filteredMenu
    : filteredMenu.slice(0, menuDisplayLimit);
  const hasMoreMenuItems = filteredMenu.length > menuDisplayLimit;
  const toggleMenuExpansion = () => {
    setMenuExpandedByFilter((current) => ({
      ...current,
      [menuExpansionKey]: !current[menuExpansionKey]
    }));
  };
      ...
      <section className="menuSection" id="menu">
        ...
        <div className="menuLayout">
          <div className="menuPanel">
            ...
            <div className="tabs limitedTabs" aria-label="Menu categories">
            ...
            <div className="tabs categoryTabs limitedTabs" aria-label="Food categories">
            ...
            <div className="menuGrid">
              {visibleMenuItems.map((item, index) => (
                <article
                  className="menuCard menuCardAnimated"
                  key={item.id}
                  role="button"
                  tabIndex={0}
              ...
              {hasMoreMenuItems && (
                ...
                >
                  {isMenuExpanded ? "View Less" : "View More"}
                </button>
              )}
            </div>
          </div>
